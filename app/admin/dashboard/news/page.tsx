      ...
      {!loading && !error && articles.length > 0 && (
        <>
              ...
              <table className="w-full">
                ...
                <tbody>
                  {paginatedArticles.map((article) => (
                    <tr key={article.id} className={`border-b hover:bg-gray-50 ${selectedIds.has(article.id) ? 'bg-green-50/50' : ''}`}>
          ...
          <div className="sm:hidden space-y-3">
            {paginatedArticles.map((article) => (
              <div key={article.id} className={`bg-white rounded-lg border p-4 space-y-2 ${selectedIds.has(article.id) ? 'ring-2 ring-green-200 bg-green-50/50' : ''}`}>
